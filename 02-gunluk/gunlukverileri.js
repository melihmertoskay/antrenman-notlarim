...
const GUNLUK_BASLANGIC=[
  ...
  {id:"2026-09-20-s1-dambil-bench-press",gun:"2026-09-20",antrenman:"S1",egzersiz:"Dambıl bench press",agirlik:"Her elde 12,5 kg",setTekrar:"3 × 10",aciklama:"Zorladı; bir sonraki S1 için hedef aynı ağırlık."},
...
const GUNLUK_ZORUNLU_GUNCELLEME_IDLERI=new Set([
  ...
  "2026-09-20-s1-incline-dambil-press",
  "2026-09-20-s1-dambil-shoulder-press",
  "2026-09-20-s1-dambil-overhead-triceps-extension"
]);
...
function gunlukKayitlariniGetir(){
  try{
    ...
    if(localStorage.getItem(GUNLUK_SURUM_ANAHTARI)!==GUNLUK_BASLANGIC_SURUMU){
      const mevcutIdler=new Map(liste.map((k,i)=>[k.id,i]));
      GUNLUK_BASLANGIC.forEach(k=>{
        const i=mevcutIdler.get(k.id);
        if(i===undefined)liste.push(gunlukKopyala(k));
        else if(GUNLUK_ZORUNLU_GUNCELLEME_IDLERI.has(k.id))liste[i]=gunlukKopyala(k);
      });
